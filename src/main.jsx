...
function SetInvitePassword({onDone}){const[p1,setP1]=useState(''),[p2,setP2]=useState(''),[busy,setBusy]=useState(false);async function save(){if(p1.length<8)return toast('كلمة المرور يجب أن تكون 8 أحرف على الأقل','error');if(p1!==p2)return toast('كلمتا المرور غير متطابقتين','error');setBusy(true);try{const{error}=await supabase.auth.updateUser({password:p1});if(error)throw error;toast('تم تفعيل الحساب وتعيين كلمة المرور');onDone()}catch(e){toast(e.message,'error')}finally{setBusy(false)}}return <div className="auth"><div className="auth-card"><div className="auth-logo">🦷</div><h1>قبول دعوة العيادة</h1><p>أنشئ كلمة المرور الخاصة بحسابك لإكمال الدخول إلى ClinicPro Dental.</p><input type="password" placeholder="كلمة المرور الجديدة" value={p1} onChange={e=>setP1(e.target.value)}/><input type="password" placeholder="تأكيد كلمة المرور" value={p2} onChange={e=>setP2(e.target.value)}/><button className="primary full" onClick={save} disabled={busy}>{busy?'جارٍ الحفظ...':'تفعيل الحساب والدخول'}</button></div></div>}

function InviteAccept({tokenHash,type='invite'}){
 ...
 async function accept(){
  ...
  try{
   ...
   if(!data?.session)throw new Error('تعذر إنشاء جلسة الدعوة')
   window.history.replaceState(null,'',window.location.pathname)
   window.location.reload()
  }catch(e){setError(e.message||'رابط الدعوة غير صالح أو منتهي الصلاحية')}
  finally{setBusy(false)}
 }
 return <div className="auth"><div className="auth-card invite-card"><div className="auth-logo">🦷</div><h1>قبول دعوة العيادة</h1><p>تمت دعوتك للانضمام إلى ClinicPro Dental. اضغط الزر أدناه لقبول الدعوة وتفعيل الحساب.</p>{error&&<div className="invite-error">{error}</div>}<button className="primary full" onClick={accept} disabled={busy}>{busy?'جاري التحقق...':'قبول الدعوة والمتابعة'}</button><small>إذا انتهت صلاحية الرابط، اطلب من مدير العيادة إرسال دعوة جديدة.</small></div></div>
}
...
function Settings({clinic,member,refresh}){const[name,setName]=useState(clinic.name),[email,setEmail]=useState(''),[role,setRole]=useState('receptionist');async function save(){const{error}=await supabase.from('clinics').update({name}).eq('id',clinic.id);if(error)toast(error.message,'error');else{toast('تم حفظ إعدادات العيادة');refresh()}}async function invite(){const{data,error}=await supabase.functions.invoke('invite-member-v2',{body:{clinic_id:clinic.id,email,role}});if(error)toast(error.message||'تعذر الاتصال بخدمة الدعوة','error');else if(data?.ok===false)toast(data.error||'تعذر تنفيذ الدعوة','error');else{toast(data?.message||'تم إرسال الدعوة');setEmail('')}}return <><Head title="الإعدادات" sub="هوية العيادة وأعضاء الفريق."/><section className="panel"><h3>هوية العيادة</h3><div className="form"><label>اسم العيادة<input value={name} onChange={e=>setName(e.target.value)}/></label></div><button className="primary" onClick={save}>حفظ</button></section>{['owner','manager'].includes(member.role)&&<section className="panel"><h3>دعوة عضو للفريق</h3><div className="invite"><input placeholder="البريد الإلكتروني" value={email} onChange={e=>setEmail(e.target.value)}/><select value={role} onChange={e=>setRole(e.target.value)}><option value="manager">مدير</option><option value="doctor">طبيب</option><option value="receptionist">استقبال</option><option value="accountant">محاسب</option></select><button className="primary" onClick={invite}>إرسال الدعوة</button></div></section>}</>}
